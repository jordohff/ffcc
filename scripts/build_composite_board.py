...
Coachspeak Index (Greg Brainos). Equal-weighted, per the user's explicit
choice (2026-08-31) - see src/ffmodel/consensus.py for the full blending
methodology and per-source caveats (scoring format, positional-only CSI).
...
DATAROMA_FILE = "ff-dataroma-redraft-rankings (1).csv"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="3 of 4 external sources are PPR (Dataroma confirmed, Hansen confirmed, Barrett assumed) - "
             "default matches the majority. CSI is half-PPR regardless of this flag (positional-only, no "
             "overall rank affected by scoring format the way point totals are).",
    )
